...
function unbind_keys() {
	...
	$(document).keydown(function(e) {
		...
		case 10009: //Back button
			parent.location = "MenuxJeux.html";
...
function PopUpManager(options) {
	var active = false;
	var html = '<div id="' + options.id + '" class="popup">' + '<header>'
			+ '<div><span>' + options.title + '</span></div>' + '</header>'
			+ '<div class="container">';
	if (options.text)
	...
	if (options.buttons) {
		for (var i = 0; i < options.buttons.length; i++) {
			$('#dim #' + options.id + ' .buttons').append(
					'<div class="' + options.buttons[i].class + '" onclick="'
							+ options.buttons[i].onclick + '">'
							+ options.buttons[i].text + '</div>');
		}
	}
	...
	return {
		...
		up : function(callback) {
			...
			if (temp_selected.index() > 0) {
				temp_selected.removeClass(focus_class).prev().addClass(
						focus_class);
				index--;
			} else {
				if (infinite) {
					temp_selected.removeClass(focus_class);
					$(list_id + ' > ' + element_list + ':last-child').addClass(
							focus_class);
		...
		down : function(callback) {
			...
			if ((temp_selected.index() + 1) < $(list_id + ' > ' + element_list).length) {
				temp_selected.removeClass(focus_class).next().addClass(
						focus_class);
				index++;
				...
				if (infinite) {
					temp_selected.removeClass(focus_class);
					$(list_id + ' > ' + element_list + ':first-child')
		...
		setIndex : function(value) {
			if ((value < $(list_id + ' > ' + element_list).length)
					|| (value > 0)) {
				$(list_id + ' > .' + focus_class).removeClass(focus_class);
				$(
						list_id + ' > ' + element_list + ':nth-child('
								+ (value + 1) + ')').addClass(focus_class);
				index = value;
			}
		},
...
popupExit = new PopUpManager({
	...
	content : '<ul id="popup-exit-list"> ' + '<li>' + '<div></div>'
			+ '<span class="button_no"></span>' + '<div></div>' + '</li>'
			+ '<li>' + '<div></div>' + '<span class="button_yes"></span>'
			+ '<div></div>' + '</li>' + '</ul>',
	onShow : function() {
